...
via `archived_at` (see `../colette-exercise-api/docs/ENGINEERING_GUIDE.md`).
...
- **To enforce capacity on register**, the check lives in `ActivityAttendance`'s
  ...
  **locks the activity row `FOR UPDATE`** so concurrent registrations serialise and can't
  ...
  (see `../colette-exercise-api/docs/GRAPHQL.md`). There is no `:activity_full` atom.
...
`../colette-exercise-api/docs/ENGINEERING_GUIDE.md`, `../colette-exercise-api/docs/GRAPHQL.md`, and `docs/API.md`.
